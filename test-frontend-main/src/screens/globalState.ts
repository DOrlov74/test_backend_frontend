...
export class GlobalState {
  ...
  get orderStatuses() {
    if (this.status.orderStatuses === null) {
      const result = client.readQuery(ORDER_STATUSES_QUERY)
  ...
  get productStatuses() {
    if (this.status.productStatuses === null) {
      const result = client.readQuery(PRODUCT_STATUSES_QUERY)
      console.log(result?.data)
  ...
  get deliveryTypes() {
    if (this.status.deliveryTypes === null) {
      const result = client.readQuery(DELIVERY_TYPES_QUERY)
      console.log(result?.data)
